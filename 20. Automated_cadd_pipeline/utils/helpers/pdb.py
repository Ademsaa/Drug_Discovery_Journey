...
import MDAnalysis as mda  # for manipulating PDB files
...
def extract_molecule_from_pdb_file(molecule_name, input_filepath, output_filepath=None):
    ...
    pdb_structure = mda.Universe(str(input_filepath))
    molecule_name = f"resname {molecule_name}" if molecule_name != "protein" else "protein"
    extracted_structure = pdb_structure.select_atoms(molecule_name)
    if output_filepath is not None:
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            extracted_structure.write(str(Path(output_filepath).with_suffix(".pdb")))
